...
internal class BLBFSLoadPackageManifestOperation : FSLoadPackageManifestOperation
{
    [UnityEngine.Scripting.Preserve]
    private enum ESteps
    {
    ...
    private RequestBiliBiliPackageHashOperation _requestRemotePackageHashOp;
    private LoadBiliBiliPackageManifestOperation _loadRemotePackageManifestOp;
    private ESteps _steps = ESteps.None;

    [UnityEngine.Scripting.Preserve]
    public BLBFSLoadPackageManifestOperation(BiliBiliFileSystem fileSystem, string packageVersion, int timeout)
    {
    ...
    public override void InternalOnUpdate()
    {
        ...
        if (_steps == ESteps.RequestRemotePackageHash)
        {
            ...
            {
                _requestRemotePackageHashOp = new RequestBiliBiliPackageHashOperation(_fileSystem, _packageVersion, _timeout);
        ...
        if (_steps == ESteps.LoadRemotePackageManifest)
        {
            if (_loadRemotePackageManifestOp == null)
            {
                ...
                _loadRemotePackageManifestOp = new LoadBiliBiliPackageManifestOperation(_fileSystem, _packageVersion, packageHash, _timeout);
